...
// Connectivity hint: enable auto-detect long polling only (not both)
// Using only experimentalAutoDetectLongPolling avoids conflict with defaults.
db.settings({
    experimentalAutoDetectLongPolling: true,
    // Merge with existing settings to avoid overriding host and suppress warning
    // @ts-ignore - 'merge' is accepted by compat layer in compat API
    merge: true,
} as any);
