...
const ToDoListView: React.FC<ToDoListViewProps> = ({ userId, user, t, getThemeClasses, showAppModal, onBack, triggerHapticFeedback }) => {
  ...
  const scheduleNotification = useCallback((task: ToDoTask) => {
    ...
    if (task.reminderAt && task.reminderAt.toDate() > new Date() && !task.completed) {
        ...
        const timeoutId = window.setTimeout(() => {
            showNativeNotification(t('notification_task_reminder_title', { taskText: task.text }), {
                body: t('notification_task_reminder_body'),
            });
            reminderTimeoutsRef.current.delete(task.id);
        }, delay);
