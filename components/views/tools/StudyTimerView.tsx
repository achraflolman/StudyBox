...
import { Play, Pause, RefreshCw, CheckCircle, Loader2, ArrowLeft } from 'lucide-react';
...
const StudyTimerView: React.FC<StudyTimerViewProps> = (props) => {
  const { 
    ...
    selectedTaskForTimer, setSelectedTaskForTimer, onBack, triggerHapticFeedback
  } = props;
  ...
  return (
    <div className="animate-fade-in">
        <div className="flex items-center mb-4">
            ...
            <h3 className={`font-bold text-xl flex-grow text-center ${getThemeClasses('text-strong')}`}>{t('pomodoros')}</h3>
      ...
      <div className={`p-4 rounded-lg shadow-inner ${getThemeClasses('bg-light')} flex flex-col items-center space-y-4`}>
          ...
          <div className="flex gap-4">
              <button onClick={() => { if (!isTimerActive) triggerHapticFeedback(); setIsTimerActive(!isTimerActive); }} className={`flex items-center gap-2 p-4 rounded-full text-white shadow-lg transition-transform active:scale-90 ${getThemeClasses('bg')}`}>
