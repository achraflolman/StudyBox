...
const securityQuestions = [
    { questionKey: 'admin_pin_secret_question_1', answer: '16 JUNI 2025' },
    { questionKey: 'admin_pin_secret_question_2', answer: 'SCHOOLMAPS' },
];
...
const PinVerificationModal: React.FC<PinVerificationModalProps> = ({ isOpen, onClose, onSuccess, t, getThemeClasses }) => {
    ...
    const [currentQuestion, setCurrentQuestion] = useState<{questionKey: string, answer: string} | null>(null);
    ...
    const handleConfirm = (e: React.FormEvent) => {
        ...
        if (answer.trim().toUpperCase() === currentQuestion.answer) {
            onSuccess();
        } else {
            ...
            <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-sm w-full transform transition-all duration-300 scale-100 animate-scale-up" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleConfirm}>
                    <label className="block text-gray-700 text-sm font-bold mb-2">
                        {currentQuestion ? t(currentQuestion.questionKey) : 'Loading question...'}
                    </label>
                    <input
                        type="text"
                        value={answer}
