...
const LoadingScreen: React.FC<{ getThemeClasses: (variant: string) => string; }> = ({ getThemeClasses }) => {
    return (<div className={`fixed inset-0 flex flex-col items-center justify-center ${getThemeClasses('bg')} z-50`}>
...
const ReauthModal: React.FC<{
    ...
    return (<div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50 animate-fade-in" onClick={onClose}>
...
const App: React.FC = () => {
    ...
    const t = useCallback((key: string, replacements: { [key: string]: string | number } = {}): string => {
        // Try translation; fallback prettifies unknown keys (remove underscores)
        let text = translations[language]?.[key] || translations['nl']?.[key] || key.replace(/_/g, ' ');
    ...
    useEffect(() => {
        ...
        const onFsError = (err: any) => {
            if (err?.code === 'permission-denied') {
                console.warn('Firestore permission denied (check security rules).', err);
            } else {
            ...
            snapshot.docChanges().forEach((change) => {
                if (change.type === 'added') {
                    ...
                    if (!existingNoteIds.has(noteId)) {
                        const newNotifRef = userNotifsRef.doc();
                        batch.set(newNotifRef, {
                            title: tRef.current('notification_note_share_title'),
                            text: tRef.current('notification_note_share_text', { name: shareData.sharerName, noteTitle: shareData.title }),
                            type: 'note_share', read: false,
        ...
        unsubscribers.push(allFilesQuery.onSnapshot((snapshot) => setAllUserFiles(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as FileData))), onFsError));
    
        const allNotesQuery = db.collection(`users/${currentUid}/notes`);
        unsubscribers.push(allNotesQuery.onSnapshot((snapshot) => setAllUserNotes(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Note))), onFsError));
    
        const allSetsQuery = db.collection(`users/${currentUid}/flashcardDecks`);
        unsubscribers.push(allSetsQuery.onSnapshot((snapshot) => setAllUserFlashcardSets(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as FlashcardSet))), onFsError));
    
        const allTasksQuery = db.collection(`users/${currentUid}/tasks`);
        unsubscribers.push(allTasksQuery.onSnapshot((snapshot) => setAllUserTasks(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as ToDoTask))), onFsError));
    
        const allSessionsQuery = db.collection(`users/${currentUid}/studySessions`);
        unsubscribers.push(allSessionsQuery.onSnapshot((snapshot) => setAllStudySessions(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as StudySession))), onFsError));
        
        const historiesQuery = db.collection(`users/${currentUid}/chatHistories`).orderBy('updatedAt', 'desc');
        unsubscribers.push(historiesQuery.onSnapshot(snapshot => setChatHistories(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as ChatHistory))), onFsError));
    ...
    const deleteStudyPlanFromAI = useCallback(async (title: string): Promise<string> => {
        return new Promise((resolve) => {
            ...
            if (!planToDelete) {
                return resolve(t('delete_study_plan_from_ai_not_found', { title }));
            }

            showAppModal({
                text: t('delete_study_plan_from_ai_confirm', { title }),
                confirmAction: async () => {
                    try {
                        const batch = db.batch();
                        ...
                        await batch.commit();
                        resolve(t('delete_study_plan_from_ai_success', { title }));
                    } catch (error) {
                        console.error("Failed to delete study plan from AI:", error);
                        resolve(t('delete_study_plan_from_ai_fail', { title }));
                    }
                },
                cancelAction: () => {
                    resolve("User cancelled the deletion.");
                }
            });
        });
    }, [user?.uid, userStudyPlans, showAppModal, t]);
    ...
    useEffect(() => {
        const authSubscriber = auth.onAuthStateChanged(async (firebaseUser) => {
            ...
            try {
                ...
                // Detailed logging to help debug permissions issues
                console.error("Error Code:", error.code);
                console.error("Error Message:", error.message);
                if (firebaseUser) {
                    console.error("Firebase UID:", firebaseUser.uid);
                    console.error("Firebase Email:", firebaseUser.email);
                }
                showAppModal({ text: `Login Error: ${error.message}. Check console for details.`});
                await auth.signOut();
            }
        });
     ...
     useEffect(() => {
        if (user && !isAdmin && process.env.API_KEY) {
            resetAIChat();
            ...
            const deleteStudyPlanTool: FunctionDeclaration = {
                name: "deleteStudyPlan", description: "Deletes a study plan by its title. The user must be asked for confirmation for this action.",
                parameters: { type: Type.OBJECT, properties: { title: { type: Type.STRING } }, required: ["title"] }
            };

            const tools: Tool[] = [{ functionDeclarations: [addEventTool, removeEventTool, getEventsTool, getStudyPlansTool, getStudyPlanDetailsTool, deleteStudyPlanTool] }];
    ...
    return (<div className={`${appContainerClasses} ${authContainerClasses}`}>
             <style>{`
            ...
            {isLoggingOut ? (<LogoutAnimationView 
                ...
                {appStatus === 'unauthenticated' && introChecked && (showIntro ? (
                ...
                {appStatus === 'authenticated' && user && (showAiSetup ? (
                        <AISetupView 
                            user={user}
                            ...
                            getThemeClasses={getThemeClasses}
                        />
                     ) : isAdmin && adminSettings ? (adminSettings.pinProtectionEnabled && !isPinVerified ? (
                            <AdminPinView 
                                user={user}
                            ...
                            />
                             ...
                             />)
                     ) : user && !isAdmin ? (<MainAppLayout {...mainAppLayoutProps} />) : <LoadingScreen getThemeClasses={getAuthThemeClasses} />
                )}
              </>
            )}
